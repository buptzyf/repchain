...
public class testErasureCodeOfHadoop {
    ...
    private static DataOfStripe[] copyOut(DataOfStripe[] out){
        ...
        for(int i = 0; i < out.length; i++){
            rds[i] = new DataOfStripe(out[i].version(),out[i].serial(),out.length,out[i].stripe(),  out[i].rootHash(), out[i].branch(),out[i].unknownFields());
        }
        return rds;
    }
    ...
    public static void main(String[] args){
        ...
        String src =  generateBigData(61200000,false);
        try {
            long start = System.currentTimeMillis();
