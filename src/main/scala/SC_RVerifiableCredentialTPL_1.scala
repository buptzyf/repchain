...
class SC_RVerifiableCredentialTPL_1 extends IContract{
  ...
  override def onAction(ctx: ContractContext, action: String, sdata: String): ActionResult = {
    ...
    action match {
      case Action.SignupCCS =>
        SignupCCS(ctx, param.extract[SC_RVerifiableCredentialTPL_1SignupCCSParam])
      case Action.UpdateCCSStatus =>
        UpdateCCSStatus(ctx, param.extract[SC_RVerifiableCredentialTPL_1UpdateCCSStatusParam])
      case Action.SignupVCStatus =>
        SignupVCStatus(ctx, param.extract[SC_RVerifiableCredentialTPL_1SignupVCStatusParam])
      case Action.UpdateVCStatus =>
        UpdateVCStatus(ctx, param.extract[SC_RVerifiableCredentialTPL_1UpdateVCStatusParam])
      case Action.RevokeVCClaims =>
        RevokeVCClaims(ctx, param.extract[SC_RVerifiableCredentialTPL_1RevokeVCClaimsParam])
      case f =>
   ...
   * @return
   */
  def SignupCCS(ctx: ContractContext, param:SC_RVerifiableCredentialTPL_1SignupCCSParam): ActionResult = {
   ...
   * @return
   */
  def UpdateCCSStatus(ctx: ContractContext, param:SC_RVerifiableCredentialTPL_1UpdateCCSStatusParam): ActionResult = {
   ...
   * @return
   */
  def SignupVCStatus(ctx: ContractContext, param:SC_RVerifiableCredentialTPL_1SignupVCStatusParam): ActionResult = {
   ...
   * @return
   */
  def UpdateVCStatus(ctx: ContractContext, param:SC_RVerifiableCredentialTPL_1UpdateVCStatusParam): ActionResult = {
   ...
   * @return
   */
  def RevokeVCClaims(ctx: ContractContext, param:SC_RVerifiableCredentialTPL_1RevokeVCClaimsParam): ActionResult = {
...
object RVerifiableCredentialTPL {
  ...
  final case class SC_RVerifiableCredentialTPL_1SignupCCSAttrParam(
                                       name: String,
                                       `type`: String,
                                       required: Boolean = true,
                                       description: String
                                     )
  final case class SC_RVerifiableCredentialTPL_1SignupCCSParam(
                                   id: String,
                                   name: String,
                                   version: String, // ccsVersion
                                   created: String,
                                   description: String,
                                   attributes: Seq[SC_RVerifiableCredentialTPL_1SignupCCSAttrParam]
                                 )
  final case class SC_RVerifiableCredentialTPL_1UpdateCCSStatusParam(id: String, valid: Boolean)

  final case class SC_RVerifiableCredentialTPL_1SignupVCStatusParam(id: String, status: String)
  final case class SC_RVerifiableCredentialTPL_1UpdateVCStatusParam(id: String, status: String)
  final case class SC_RVerifiableCredentialTPL_1RevokeVCClaimsParam(id: String, revokedClaimIndex: Seq[String])
  ...
  def assertSignupCCSParam(param:SC_RVerifiableCredentialTPL_1SignupCCSParam) = {
  ...
  def assertUpdateCCSStatusParam(param:SC_RVerifiableCredentialTPL_1UpdateCCSStatusParam) = {
    assertRequiredParamStrField("id", param.id)
  }

  def assertSignupVCStatusParam(param:SC_RVerifiableCredentialTPL_1SignupVCStatusParam) = {
    assertRequiredParamStrField("id", param.id)
    assertRequiredParamStrField("status", param.status)
  }
  def assertUpdateVCStatusParam(param:SC_RVerifiableCredentialTPL_1UpdateVCStatusParam) = {
    assertRequiredParamStrField("id", param.id)
    assertRequiredParamStrField("status", param.status)
  }
  def assertRevokeVCClaimsParam(param:SC_RVerifiableCredentialTPL_1RevokeVCClaimsParam) = {
    assertRequiredParamStrField("id", param.id)
  ...
  def assertRequiredParamSeqSignupCCSAttrField(fieldName: String, fieldValue: Seq[SC_RVerifiableCredentialTPL_1SignupCCSAttrParam]): Null = {
    if (fieldValue.length == 0) {
      throw ContractException(
        JsonFormat.toJsonString(
          ActionResult(STATUS_CODE_BAD_REQUEST, s"参数字段${fieldName}不能为空数组")
