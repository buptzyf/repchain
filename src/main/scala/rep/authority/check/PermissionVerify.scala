...
class PermissionVerify(sysTag:String) {
  ...
  //did，opname必填
  def CheckPermission(did:String,opname:String,dbinstance:ImpDataPreload): Boolean ={
    var r = false
  ...
  def CheckPermissionOfCertId(certid:CertId, opname:String, dbinstance:ImpDataPreload): Boolean ={
    CheckPermission(certid.creditCode,opname,dbinstance)
  }
  ...
  def CheckPermissionOfCertHash(certhash:String, opname:String, dbinstance:ImpDataPreload): Boolean ={
    CheckPermissionOfCertId(IdTool.getCertIdFromName(certCache.getCertIdForHash(certhash,sysTag)), opname, dbinstance)
  }

  def CheckPermissionOfDeployContract(dotrans: DoTransactionOfSandbox,shim:Shim):Boolean={
    var r = true
  ...
  def CheckPermissionOfSetStateContract(dotrans: DoTransactionOfSandbox,shim:Shim)={
    var r = true
  ...
  def CheckPermissionOfInvokeContract(dotrans: DoTransactionOfSandbox,shim:Shim)={
    val cid = dotrans.t.cid.get
