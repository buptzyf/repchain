...
import rep.proto.rc2.{Block, ChaincodeDeploy, ChaincodeId, Operate, Transaction}
...
object CreateGenesisInfoInGM {
  ...
  def main(args: Array[String]): Unit = {
    ...
    for (i <- 0 to 5) {
      ...
      //生成Did的身份证书
      val authcert = rep.proto.rc2.Certificate(certstr, "SHA256withECDSA", true, Option(Timestamp(millis / 1000, ((millis % 1000) * 1000000).toInt)),
        _root_.scala.None, CertType.CERT_AUTHENTICATION, Option(certid), certstrhash, "1.0")
      val signer_tmp = rep.proto.rc2.Signer(nodes(i)._1, nodes(i)._2, nodes(i)._3, _root_.scala.Seq.empty,
    ...
    for (i <- 0 to 13) {
      ...
      if (i == 3 || i == 4 || i ==7 || i == 8 || i == 9 || i ==10 || i == 11) {
        ...
        op = rep.proto.rc2.Operate(opsOfContract(i)._1, opsOfContract(i)._2, super_credit, true, OperateType.OPERATE_CONTRACT,
          snls, "*", opsOfContract(i)._3, Option(Timestamp(millis / 1000, ((millis % 1000) * 1000000).toInt)),
          _root_.scala.None, true, "1.0")
      } else {
        op = rep.proto.rc2.Operate(opsOfContract(i)._1, opsOfContract(i)._2, super_credit, false, OperateType.OPERATE_CONTRACT,
          snls, "*", opsOfContract(i)._3, Option(Timestamp(millis / 1000, ((millis % 1000) * 1000000).toInt)),
          _root_.scala.None, true, "1.0")
      }
    ...
    val at = rep.proto.rc2.Authorize(IdTool.getRandomUUID, super_credit, granteds, opids,
    ...
    val dep_asserts_trans = ctx.getTransactionBuilder.createTransaction4Deploy(sysName, cid2, c2, "", 5000, CodeType.CODE_SCALA,RunType.RUN_SERIAL,StateType.STATE_BLOCK,
      ChaincodeDeploy.ContractClassification.CONTRACT_SYSTEM,0)
    ...
    val op1 = rep.proto.rc2.Operate(opsOfCustomContract(0)._1, opsOfCustomContract(0)._2, sys_credit, true, OperateType.OPERATE_CONTRACT,
    ...
    val op2 = rep.proto.rc2.Operate(opsOfCustomContract(1)._1, opsOfCustomContract(1)._2, sys_credit, false, OperateType.OPERATE_CONTRACT,
    ...
    val op3 = rep.proto.rc2.Operate(opsOfCustomContract(2)._1, opsOfCustomContract(2)._2, sys_credit, true, OperateType.OPERATE_CONTRACT,
    ...
    translist += ctx.getTransactionBuilder.createTransaction4Invoke("257091603041653856.super_admin", cid2, "set", Seq(ct1))
    ...
    for (i <- 0 to 3) {
      val coop_op = Operate(opsOfCoopContract(i)._1, opsOfCoopContract(i)._2, super_credit, true, OperateType.OPERATE_CONTRACT,
    ...
    val pw = new PrintWriter("json/${ctx.getConfig.getChainNetworkId}/genesis_gm.json", "UTF-8")
    pw.write(rstr)
    pw.flush()
    pw.close()
  }
