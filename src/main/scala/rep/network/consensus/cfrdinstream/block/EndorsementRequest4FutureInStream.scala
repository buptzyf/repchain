...
class EndorsementRequest4FutureInStream (moduleName: String) extends ModuleBase(moduleName) {
  ...
  private def ExecuteOfEndorsement(addr: Address, data: EndorsementInfoInStream): ResultOfEndorsed = {
    try {
      ...
      RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix( s"--------EndorsementRequest4FutureInStream waiting resultheight=${data.blc.height},local height=${pe.getCurrentHeight}"))
      Await.result(future1, timeout.duration).asInstanceOf[ResultOfEndorsed]
    } catch {
      case e: AskTimeoutException =>
        RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix( s"--------EndorsementRequest4FutureInStream timeout,height=${data.blc.height},local height=${pe.getCurrentHeight}"))
        null
      case te: TimeoutException =>
        RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix( s"--------EndorsementRequest4FutureInStream java timeout,height=${data.blc.height},local height=${pe.getCurrentHeight}"))
        null
    }
  }
    ...
    if (result != null) {
      if (result.result == ResultFlagOfEndorse.success) {
        if (EndorsementVerify(reqinfo.blc, result)) {
          ...
          context.parent ! re
          RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix( s"--------EndorsementRequest4FutureInStream, send endorsement, height=${reqinfo.blc.height},local height=${pe.getCurrentHeight} "))
        } else {
          RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix(s"--------EndorsementRequest4FutureInStream recv endorsement result is error, result=${result.result},height=${reqinfo.blc.height},local height=${pe.getCurrentHeight}"))
        ...
        if (result.result == ResultFlagOfEndorse.BlockHeightError) {
          if (result.endorserOfChainInfo.height > pe.getCurrentHeight + 1) {
            ...
            context.parent ! ResultOfEndorseRequester(false, null, reqinfo.blc.hashOfBlock.toStringUtf8(), reqinfo.endorer)
            RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix( s"--------EndorsementRequest4FutureInStream recv endorsement result must synch,height=${reqinfo.blc.height},local height=${pe.getCurrentHeight} "))
          } else {
      ...
      RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix(s"--------EndorsementRequest4FutureInStream recv endorsement result is null,height=${reqinfo.blc.height},local height=${pe.getCurrentHeight} "))
      //context.parent ! ResultOfEndorseRequester(false, null, reqinfo.blc.hashOfBlock.toStringUtf8(), reqinfo.endorer)
      context.parent ! ResendEndorseInfo(reqinfo.endorer)
    }
    ...
    case RequesterOfEndorsementInStream(block, blocker, addr,voteindex,voteheiht) =>
      ...
      if(NodeHelp.isBlocker(pe.getSysTag, pe.getBlocker.blocker)){
        if(isAcceptEndorseRequest(pe.getBlocker, block)){
          ...
          RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix(s"--------EndorsementRequest4FutureInStream back out  endorsement,prehash not equal  ,height=${block.height},local height=${pe.getCurrentHeight} "))
        }
      }
    case _ => //ignore
  }
