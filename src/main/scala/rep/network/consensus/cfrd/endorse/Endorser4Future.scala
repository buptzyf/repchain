...
class Endorser4Future(moduleName: String) extends ModuleBase(moduleName) {
  ...
  private def AskPreloadTransactionOfBlock(block: Block): Boolean = {
    ...
    try {
      ...
      var tmpblock = result.blc.withHeader(result.blc.header.get.withHashPresent(block.header.get.hashPresent))
  ...
  private def isAllowEndorse(info: EndorsementInfo): Int = {
    ...
    if (info.blocker.blocker == pe.getSysTag) {
        ...
        if (NodeHelp.isCandidateNow(pe.getSysTag, SystemCertList.getSystemCertList)) {
          ...
          if (info.blc.header.get.hashPrevious.toStringUtf8 == pe.getBlocker.voteBlockHash && NodeHelp.isBlocker(info.blocker.blocker, pe.getBlocker.blocker)) {
            //可以进入背书
            RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix( s"vote result equal，allow entry endorse,recv endorse request,endorse height=${info.blc.header.get.height},local height=${pe.getCurrentHeight}"))
            0
          } else  {
            //todo 需要判断区块缓存，再决定是否需要启动同步,并且当前没有同步才启动同步，如果已经同步，则不需要发送消息。
            if(info.blc.header.get.height > pe.getCurrentHeight+1){
              if(!pe.isSynching){
                ...
                RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix( s"vote index not equal，,endorse height=${info.blc.header.get.height},local height=${pe.getCurrentHeight},blocker index=${info.blocker.voteIndex},local index=${pe.getBlocker.VoteIndex}"))
                pe.getActorRef(CFRDActorType.ActorType.voter) ! SpecifyVoteHeight(info.blocker)
              } else{
                RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix( s"node is synchoronizing,recv endorse request,endorse height=${info.blc.header.get.height},local height=${pe.getCurrentHeight}"))
              }
              //本地正在同步
              2
            }else if(info.blc.header.get.hashPresent.toStringUtf8 == pe.getCurrentBlockHash){
              if(pe.getBlocker.blocker == ""){
                RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix( s"local not vote,start vote,recv endorse request,endorse height=${info.blc.header.get.height},local height=${pe.getCurrentHeight}"))
                ...
                RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix( s"do not endorsed,vote index not equal,reset vote,recv endorse request,endorse height=${info.blc.header.get.height},local height=${pe.getCurrentHeight}"))
                //重置抽签
                5
              }else{
                RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix( s"do not endorsed,unknow of reason,recv endorse request,endorse height=${info.blc.header.get.height},local height=${pe.getCurrentHeight}"))
                6
              }
            }else{
              RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix( s"do not endorsed,unknow of reason,recv endorse request,endorse height=${info.blc.header.get.height},local height=${pe.getCurrentHeight}"))
  ...
  private def isRepeatEndoresment(info: EndorsementInfo):Boolean={
    ...
    if(this.blockOfEndorement != null && this.resultOfEndorement != null) {
      if( info.blc.header.get.hashPresent.toStringUtf8 == this.blockOfEndorement.header.get.hashPresent.toStringUtf8 && info.blocker == this.resultOfEndorement.endorserOfVote.blocker){
        r = true
      }else{
    ...
    r match {
      case 0 =>
        ...
        if (SystemProfile.getIsVerifyOfEndorsement) {
          ...
          RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix(s"${pe.getSysTag}:entry 8"))
          sender ! ResultOfEndorsed(ResultFlagOfEndorse.VerifyError, null, info.blc.header.get.hashPresent.toStringUtf8(),
            pe.getSystemCurrentChainStatus,pe.getBlocker)
        }
      case 2 =>
        //cache endorse,waiting revote
        sender ! ResultOfEndorsed(ResultFlagOfEndorse.BlockHeightError, null, info.blc.header.get.hashPresent.toStringUtf8(), pe.getSystemCurrentChainStatus, pe.getBlocker)
        RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix(s"endorse node not equal height,synching,self height=${pe.getCurrentHeight},block height=${info.blc.header.get.height}"))
      case 4=>
        sender ! ResultOfEndorsed(ResultFlagOfEndorse.EndorseNodeNotVote, null, info.blc.header.get.hashPresent.toStringUtf8(), pe.getSystemCurrentChainStatus, pe.getBlocker)
        RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix(s"do not vote,force vote,self height=${pe.getCurrentHeight},block height=${info.blc.header.get.height}"))
      case 5=>
        sender ! ResultOfEndorsed(ResultFlagOfEndorse.VoteIndexError, null, info.blc.header.get.hashPresent.toStringUtf8(), pe.getSystemCurrentChainStatus, pe.getBlocker)
        RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix(s"vote index error,reset vote,self height=${pe.getCurrentHeight},block height=${info.blc.header.get.height}"))
      case 6=>
        sender ! ResultOfEndorsed(ResultFlagOfEndorse.EndorseNodeUnkonwReason, null, info.blc.header.get.hashPresent.toStringUtf8(), pe.getSystemCurrentChainStatus, pe.getBlocker)
        RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix(s"endorsement Unknow reason,self height=${pe.getCurrentHeight},block height=${info.blc.header.get.height}"))
      case 1 =>
        //do not endorse
        sender ! ResultOfEndorsed(ResultFlagOfEndorse.BlockerSelfError, null, info.blc.header.get.hashPresent.toStringUtf8(), pe.getSystemCurrentChainStatus, pe.getBlocker)
        RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix(s"itself,recv endorse request,endorse height=${info.blc.header.get.height},local height=${pe.getCurrentHeight}"))
      case 3 =>
        //do not endorse
        sender ! ResultOfEndorsed(ResultFlagOfEndorse.CandidatorError, null, info.blc.header.get.hashPresent.toStringUtf8(), pe.getSystemCurrentChainStatus, pe.getBlocker)
        RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix(s"it is not candator,do not endorse,recv endorse request,endorse height=${info.blc.header.get.height},local height=${pe.getCurrentHeight}"))
    }
  }
