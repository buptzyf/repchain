...
class EndorseCollector(moduleName: String) extends ModuleBase(moduleName) {
  ...
  private def CheckAndFinishHandler {
    ...
    if (ConsensusCondition.ConsensusConditionChecked(this.recvedEndorse.size + 1)) {
      ...
      this.block.withHeader( this.block.header.get.withEndorsements(consensus))
      RepTimeTracer.setEndTime(pe.getSysTag, "Endorsement", System.currentTimeMillis(),this.block.header.get.height,this.block.transactions.size)
  ...
  override def receive = {
    ...
    case CollectEndorsement(block, blocker) =>
      if(!pe.isSynching && ConsensusCondition.CheckWorkConditionOfSystem(pe.getNodeMgr.getStableNodes.size)) {
        ...
        if (this.block != null && this.block.header.get.hashPresent.toStringUtf8() == block.header.get.hashPresent.toStringUtf8()) {
          //需要重启背书
          ...
          if(this.blocker.voteIndex < blocker.voteIndex && blocker.blocker == this.blocker.blocker){
            if (block.header.get.hashPrevious.toStringUtf8() == pe.getCurrentBlockHash) {
              RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix(s"collectioner recv endorsement in repeat endorsement,height=${block.header.get.height},local height=${pe.getCurrentHeight}"))
              //resetEndorseInfo(block, blocker, index)
              resetEndorseInfo(block, blocker)
              pe.getNodeMgr.getStableNodes.foreach(f => {
                RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix(s"collectioner send endorsement to requester in repeat endorsement,height=${block.header.get.height},local height=${pe.getCurrentHeight}"))
              ...
              RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix(s"collectioner back out endorsement request in repeat endorsement,height=${block.header.get.height},local height=${pe.getCurrentHeight}"))
            }
          }
          RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix(s"collectioner is waiting endorse result in repeat endorsement,height=${block.header.get.height},local height=${pe.getCurrentHeight}"))
        } else {
          //第一次背书
          if (block.header.get.hashPrevious.toStringUtf8() == pe.getCurrentBlockHash) {
            RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix(s"collectioner recv endorsement in first endorsement,height=${block.header.get.height},local height=${pe.getCurrentHeight}"))
            //resetEndorseInfo(block, blocker, index)
            resetEndorseInfo(block, blocker)
            pe.getNodeMgr.getStableNodes.foreach(f => {
              RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix(s"collectioner send endorsement to requester in first endorsement,height=${block.header.get.height},local height=${pe.getCurrentHeight}"))
              //router.route(RequesterOfEndorsement(block, blocker, f, pe.getBlocker.VoteIndex), self)
              router.route(RequesterOfEndorsement(block, blocker, f), self)
            })
          } else {
            RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix(s"collectioner back out endorsement request in first endorsement,height=${block.header.get.height},local height=${pe.getCurrentHeight}"))
          }
        }
      }else{
        RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix(s"collectioner do not send endorsement request ,reason:synch?;nodes too little,height=${block.header.get.height},local height=${pe.getCurrentHeight}"))
      }
        /*//第一次背书和重启背书采用同一逻辑
    ...
    case ResultOfEndorseRequester(result, endors, blockhash, endorser) =>
      if(!pe.isSynching){
        ...
        if (this.block != null && this.block.header.get.hashPrevious.toStringUtf8() == pe.getCurrentBlockHash && this.block.header.get.hashPresent.toStringUtf8() == blockhash) {
            if (result) {
              RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix( s"collectioner recv endorsement result,height=${block.header.get.height},local height=${pe.getCurrentHeight}"))
              recvedEndorse += endorser.toString -> endors
              CheckAndFinishHandler
            } else {
              RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix(s"collectioner recv endorsement result,is error,height=${block.header.get.height},local height=${pe.getCurrentHeight}"))
            }
        }else{
    ...
    case ResendEndorseInfo(endorer)=>
      if(!pe.isSynching && ConsensusCondition.CheckWorkConditionOfSystem(pe.getNodeMgr.getStableNodes.size)){
        if (this.block != null && this.block.header.get.hashPrevious.toStringUtf8() == pe.getCurrentBlockHash ) {
          if(this.router != null){
            if(this.resendTimes <= SystemProfile.getEndorseResendTimes){
              if(this.resendEndorsements.isEmpty){
                this.schedulerLink = clearSched()
                schedulerLink = scheduler.scheduleOnce(( TimePolicy.getTimeoutEndorse * 2 ).second, self, DelayResendEndorseInfo(this.block.header.get.hashPresent.toStringUtf8))
              }
              this.resendEndorsements += endorer
            }else{
              RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix( s"ResendEndorseInfo endorse info,resend times eq ${this.resendTimes} ,height=${block.header.get.height},local height=${pe.getCurrentHeight}"))
            }
          }else{
            RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix( s"ResendEndorseInfo collectioner's router is null,height=${block.header.get.height},local height=${pe.getCurrentHeight}"))
          }
        }else{
    ...
    case DelayResendEndorseInfo(bHash)=>
      this.schedulerLink = clearSched()
      if(!pe.isSynching && ConsensusCondition.CheckWorkConditionOfSystem(pe.getNodeMgr.getStableNodes.size)){
        if (this.block != null && bHash == this.block.header.get.hashPresent.toStringUtf8 && this.block.header.get.hashPrevious.toStringUtf8() == pe.getCurrentBlockHash ) {
          if(this.router != null){
            if(this.resendTimes <= SystemProfile.getEndorseResendTimes){
              this.resendTimes += 1
              ...
              RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix( s"DelayResendEndorseInfo endorse info,resend times eq ${this.resendTimes} ,height=${block.header.get.height},local height=${pe.getCurrentHeight}"))
            }

          }else{
            RepLogger.trace(RepLogger.Consensus_Logger, this.getLogMsgPrefix( s"DelayResendEndorseInfo collectioner's router is null,height=${block.header.get.height},local height=${pe.getCurrentHeight}"))
          }
        }else{
