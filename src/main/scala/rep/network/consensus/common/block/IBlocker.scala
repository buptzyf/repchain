...
abstract class IBlocker(moduleName: String) extends ModuleBase(moduleName) {
  ...
  protected val blockSearch: BlockSearcher = new BlockSearcher(pe.getSysTag)
  ...
  protected def ExecuteTransactionOfBlock(block: Block): Block = {
    try {
      ...
      BlockPreload.freeInstance("preload-"+block.transactions(0).id,pe.getSysTag)
    }
  }
  ...
  protected def PackedBlock(start: Int = 0): Block = {
    ...
    val trans = pe.getTransactionPool.packageTransactionToBlock.reverse
    //todo 交易排序
