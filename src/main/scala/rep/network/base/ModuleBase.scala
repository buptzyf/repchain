...
abstract class  ModuleBase(name: String) extends ClusterActor{
  ...
  //预执行的超时时间
  protected implicit val preload_timeout = Timeout((pe.getRepChainContext.getTimePolicy.getTimeoutPreload * 3).seconds)
  //背书超时时间
  protected implicit val endorse_timeout = Timeout((pe.getRepChainContext.getTimePolicy.getTimeoutEndorse * 3).seconds)
  //出块超时时间
  protected implicit val block_timeout = Timeout((pe.getRepChainContext.getTimePolicy.getTimeOutBlock * 3).seconds)
  //同步超时时间
  protected implicit val sync_chain_info_timeout = Timeout((pe.getRepChainContext.getTimePolicy.getTimeoutSync * 3).seconds)
