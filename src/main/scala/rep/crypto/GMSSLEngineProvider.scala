...
class GMSSLEngineProvider (protected val config: Config, protected val log: MarkerLoggingAdapter)
  extends SSLEngineProvider
{
  def this(system: ActorSystem) =
    this(
      system.settings.config.getConfig("akka.remote.artery.ssl.config-ssl-engine"),
      Logging.withMarker(system, classOf[GMSSLEngineProvider].getName))

  private val keyStoreType = "PKCS12"
  private val jceProviderName = "GMJCE"
  private val jsseProviderName = "GMJSSE"
  System.out.println("%%%%%%%%%%%%%%%%")
  loadProvider("cn.gmssl.jce.provider.GMJCE",1)
  loadProvider("cn.gmssl.jsse.provider.GMJSSE",2)
  System.out.println("%%%%%%%%%%%%%%%%")
