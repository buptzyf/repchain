...
/**
  ...
  */
final case class SC_InterfaceCooperation_1ApiDefinition(id: String, `type`: String, version: String, name: String, algo_hash: String, algo_sign: String, para: Option[String],
...
/**
  ...
  */
final case class SC_InterfaceCooperation_1ApiServAndAck(id: String, name: String, d_id: String, version: String, e_id: String, addr: String, port: Int)
...
/**
  ...
  * @param sign       数字签名
  */
final case class SC_InterfaceCooperation_1Signature(eid: String, cert_name: String, hash: String, timeCreate: Long, sign: String)
...
/**
  ...
  */
final case class SC_InterfaceCooperation_1ReqAckProof(cid: String, e_from: String, e_to: String, method: String, b_req: Boolean, b_end: Boolean, seq: Int,
                             hash: String, hash_claim: Option[String], tm_create: Long, sign_r:SC_InterfaceCooperation_1Signature, sign_c:SC_InterfaceCooperation_1Signature)

/**
  * @author zyf
  */
class SC_InterfaceCooperation_1 extends IContract{
  ...
  /**
    ...
    * @return
    */
  def registerApiDefinition(ctx: ContractContext, apiDefinition:SC_InterfaceCooperation_1ApiDefinition): ActionResult = {
  ...
  /**
    ...
    * @return
    */
  def registerApiService(ctx: ContractContext, apiServAndAck:SC_InterfaceCooperation_1ApiServAndAck): ActionResult = {
    val txr_credit_code = ctx.t.getSignature.getCertId.creditCode
  ...
  /**
    ...
    * @return
    */
  def registerApiAckReceive(ctx: ContractContext, apiServAndAck:SC_InterfaceCooperation_1ApiServAndAck): ActionResult = {
    val txr_credit_code = ctx.t.getSignature.getCertId.creditCode
    val def_holder_key = defVersionPrefix + infix + apiServAndAck.d_id
  ...
  /**
    ...
    * @return
    */
  def reqAckProof(ctx: ContractContext, reqAckProof:SC_InterfaceCooperation_1ReqAckProof): ActionResult = {
    val txr_credit_code = ctx.t.getSignature.getCertId.creditCode
    // 应答注册者
    ...
    if (service_holder != null && ack_holder != null) {
      ...
      val servAndAck_receive = read[SC_InterfaceCooperation_1ApiServAndAck](ack_receive.asInstanceOf[String])
      // 服务方注册的接口服务
      val api_service = ctx.api.getVal(apiServPrefix + infix + service_holder.asInstanceOf[String] + infix + reqAckProof.e_to)
      val servAndAck_service = read[SC_InterfaceCooperation_1ApiServAndAck](api_service.asInstanceOf[String])
      if (reqAckProof.b_req) {
        ...
        if (servAndAck_receive.e_id == txr_credit_code) {
          ...
          if (servAndAck_receive.e_id == reqAckProof.sign_r.eid && servAndAck_service.e_id == reqAckProof.sign_c.eid) {
            ...
            val api_def = read[SC_InterfaceCooperation_1ApiDefinition](ctx.api.getVal(def_key).asInstanceOf[String])
  ...
  override def onAction(ctx: ContractContext, action: String, sdata: String): ActionResult = {
    ...
    try {
      action match {
        // 接口定义及相关登记
        case ACTION.InterfaceRegister.registerApiDefinition => registerApiDefinition(ctx, json.extract[SC_InterfaceCooperation_1ApiDefinition])
        // 接口服务登记
        case ACTION.InterfaceRegister.registerApiService => registerApiService(ctx, json.extract[SC_InterfaceCooperation_1ApiServAndAck])
        // 接口应答登记
        case ACTION.InterfaceRegister.registerApiAckReceive => registerApiAckReceive(ctx, json.extract[SC_InterfaceCooperation_1ApiServAndAck])
        // 请求应答存证
        case ACTION.InterfaceReqAckProof.reqAckProof => reqAckProof(ctx, json.extract[SC_InterfaceCooperation_1ReqAckProof])
        // 未匹配到的
